---
title: "Bio201 Final Project Fiber_high"
...
Since we got significant difference from BRMPS consumption in the BRMPS Rmd, In this Rmd we will be testing how a fixed routine of high fiber daily with the consumption of BRMPS will effect once gut microbiomal commnity in regard of richness and simpson diversity index, and beta divesity.We selceted for quantity compliant.
...
richness_df_fib <- physq_obj %>%
  ...
  subset_samples(., fiber_status == "high") %>% #subset for weeks of interest
...
# save richness data 
write_delim(richness_df_fib, path = "~/Documents/Bio201ThursPM/curated_data/richness_df_fib_high.txt", delim = "\t")
...
rich1_fib <- physq_obj %>%
  subset_samples(supplement_consumed == "BRMPS")%>%
  subset_samples(quantity_compliant == "yes") %>%
  subset_samples(., fiber_status == "high") %>% 
  subset_samples(., study_week == "week1" | study_week == "week3") %>%
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/rich2_fib_high.pdf",
          plot = rich2_fib,
          nrow = 1, ncol = 2,
...
wilcox.test(x = richness_df2_fib$richness_wk1_fib, 
       y = richness_df2_fib$richness_wk3_fib,
       alternative = "two.sided", paired = TRUE, var.equal = TRUE)

```
p-value =0.03403,  signicant difference in richness when consuming BRMPS when fiber status is fixed to a high level.(not reliable since sample size is so small)
...
# save Simpson data 
write_delim(simpson_df_fib, path = "~/Documents/Bio201ThursPM/curated_data/simpson_df_fib_high.txt", delim = "\t")
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/simp2_fib_high.pdf",
          plot = simp2_fib,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
```
p-value =0.01709,  signicant difference in simpson when consuming BRMPS when fiber status is fixed to a high level.(not reliable since sample size is so small)
...
save_plot(filename = "~/Documents/Bio201ThursPM/figures/bc_fib_high.pdf",
          plot = ordplot1,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
ordplot2 <- plot_ordination(physeq = physq_sub, 
                     ...
                     shape = "fiber_status")
print(ordplot2)
save_plot(filename = "~/Documents/Bio201ThursPM/figures/ja_fib_high.pdf",
          plot = ordplot2,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
p-value of 0.987 indicates the groups are not statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.01234 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. the change in the gut microbiome community between Wk1 & Wk3 is not significantly different in people who have a high calorie intake. Thus against our hypothesis.
...
p-value of 0.997 indicates the groups are not statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.01558 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. the change in the gut microbiome community between Wk1 & Wk3 is not significantly different in people who have a high calorie intake. Thus against our hypothesis.
-----
end
