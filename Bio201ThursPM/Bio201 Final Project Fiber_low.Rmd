---
title: "Bio201 Final Project Fiber_low"
...
Since we got significant difference from BRMPS consumption in the BRMPS Rmd, In this Rmd we will be testing how a fixed routine of low fiber daily with the consumption of BRMPS will effect once gut microbiomal commnity in regard of richness and simpson diversity index, and beta divesity.We selceted for quantity compliant.
...
# save richness data 
write_delim(richness_df_fib, path = "~/Documents/Bio201ThursPM/curated_data/richness_df_fib_low.txt", delim = "\t")
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/rich2_fib_low.pdf",
          plot = rich2_fib,
          nrow = 1, ncol = 2,
...
p-value = 0.1024, no signicant difference in richness when consuming BRMPS when fiber status is fixed to a low level.
...
# save Simpson data 
write_delim(simpson_df_fib, path = "~/Documents/Bio201ThursPM/curated_data/simpson_df_fib_low.txt", delim = "\t")
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/simp2_fib_low.pdf",
          plot = simp2_fib,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
```
p-value = 0.001, signicant difference on simpson when consuming BRMPS when fiber status is fixed to a low level.
...
save_plot(filename = "~/Documents/Bio201ThursPM/figures/bc_fib_low.pdf",
          plot = ordplot1,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
ordplot2 <- plot_ordination(physeq = physq_sub, 
                     ...
                     shape = "fiber_status")
print(ordplot2)
save_plot(filename = "~/Documents/Bio201ThursPM/figures/ja_fib_low.pdf",
          plot = ordplot2,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
```
p-value of 0.002 indicates the groups are statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.013 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test.  
...
```
p-value of 0.006 indicates the groups are statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.00946 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test.  
-----
end
