---
title: "Bio201 Final Project Calorie_low"
...
richness_df_cal <- physq_obj %>%
  ...
  subset_samples(cal_status == "low") %>% #subset for weeks of interest
...
# save richness data 
write_delim(richness_df_cal, path = "~/Documents/Bio201ThursPM/curated_data/richness_df_cal_low.txt", delim = "\t")
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/rich2_cal_low.pdf",
          plot = rich2_cal,
          nrow = 1, ncol = 2,
...
# save Simpson data 
write_delim(simpson_df_cal, path = "~/Documents/Bio201ThursPM/curated_data/simpson_df_cal_low.txt", delim = "\t")
...
# save plot
save_plot(filename = "~/Documents/Bio201ThursPM/figures/simp2_cal_low.pdf",
          plot = simp2_cal,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
```
p-value = 0.004256 significant difference in simpson when consumpetion is BRMPS AND calorie status is fixed to low level（Control variable）.
...
save_plot(filename = "~/Documents/Bio201ThursPM/figures/bc_cal_low.pdf",
          plot = ordplot1,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
ordplot2 <- plot_ordination(physeq = physq_sub, 
                     ...
                     shape = "cal_status")
print(ordplot2)
save_plot(filename = "~/Documents/Bio201ThursPM/figures/ja_cal_low.pdf",
          plot = ordplot2,
          nrow = 1, ncol = 2,
          base_aspect_ratio = 1.1) 
...
p-value of 0.001 indicates the groups are statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.012 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test.  
...
p-value of 0.001 indicates the groups are statistically significant. The R-squared value can be interpretd as the effect size, it shows the percentage of variation explained by the grouping categories.an R value of 0.00891 here indicates no significant dissimilarity between the groups, that the grouping of samples based by study week are weak. When this occurs (low p and low R), the variation of the samples in the tested groups likely is influencing the results of the test.  
