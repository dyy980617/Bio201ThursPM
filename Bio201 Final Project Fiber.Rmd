...
Research Question: 
How does the consumption of a prebiotic effect the gut microbiomal community(diversity, changes in taxon abundance) among people how have different ?
...
diet_all <- dplyr::union(diet_1, diet_w17_ASA2)
low_fiber <- filter(diet_all, Fiber_g < 31) %>% 
  rename_all(tolower) 
```
...
all_data_wkly_lowfib <- inner_join(x = all_data_wkly, y = low_fiber, 
                                  by = c( "participant_id")) 
  
all_data_wkly_lowfib$participant_id_week = paste(all_data_wkly_lowfib$participant_id,
                                                 all_data_wkly_lowfib$study_week, 
                                                 sep="_")  #match with "sv_table"

all_data_wkly_inner <- inner_join(x = all_data_wkly_lowfib, y = species_avg_shared, 
